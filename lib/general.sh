...
# function: cycle trough distributions
	...
	local distributions=("stretch" "bionic" "buster" "bullseye" "focal" "hirsute" "impish" "jammy" "bookworm" "sid")
...
# "purge" leave only last 5 versions
	...
	local DISTROS=("stretch" "bionic" "buster" "bullseye" "focal" "hirsute" "impish" "jammy" "sid" "bookworm")
...
prepare_host()
{
  ...
  if [[ $(dpkg --print-architecture) == amd64 ]]; then
	...
	if [[ $HOSTRELEASE =~ ^(focal|impish|hirsute|jammy|bookworm|ulyana|ulyssa|bullseye|uma|una)$ ]]; then
	...
	if [[ -z $HOSTRELEASE || "buster bullseye focal impish hirsute jammy bookworm debbie tricia ulyana ulyssa uma una" != *"$HOSTRELEASE"* ]]; then
